...
export default class SrpDao {
  ...
  async listReimbursements(db: Tnex, filter: SrpReimbursementFilter) {
    ...
    if (filter.account != undefined) {
      subquery = subquery
        .join(ownership, "ownership_character", "=", "srpr_recipientCharacter")
