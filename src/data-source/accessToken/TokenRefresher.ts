...
export class TokenRefresher {
  ...
  private async _refreshAccessToken(
      row: RowToRefresh): Promise<RefreshResult> {
    ...
    try {
      const response =
          await this._postRefreshRequest(row.accessToken_refreshToken);
      ...
      if (e.response) {
        if (e.response.status == 400) {
          logger.info(`Access token refresh request was rejected for char `
          ...
          logger.error(`HTTP error while refreshing token for `
              + `${row.accessToken_character}.`);
          logger.error(e);
        }
      } else {
        result.errorType = AccessTokenErrorType.HTTP_FAILURE;
        logger.error(`Generic error while refreshing token for `
            + `${row.accessToken_character}.`);
        logger.error(e);
      }
    }
